...
CREATE TABLE IF NOT EXISTS `sys_early_warning_email` (
    `id` bigint(20) NOT NULL COMMENT '主键',
    `host` varchar(100) DEFAULT NULL COMMENT '邮箱服务地址',
    `protocol` varchar(100) DEFAULT NULL COMMENT '邮箱服务',
    `port` int(11) DEFAULT NULL COMMENT '邮箱服务端口',
    `username` varchar(100) DEFAULT NULL COMMENT '邮箱账号',
    `password` varchar(100) DEFAULT NULL COMMENT '邮箱密码',
    `receivers` varchar(400) DEFAULT NULL COMMENT '邮件接收人',
    `create_time` datetime DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `update_time` datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    PRIMARY KEY (`id`) USING BTREE
